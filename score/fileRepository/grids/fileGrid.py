...
from fileRepository._fileRepository_orm import fileCursor, fileCounterCursor, fileVersionCursor
...
def gridData(context, main, add=None, filterinfo=None,
    ...
    for files in file_cursor.iterate():
        rec = {}
        rec[_header["id"][-1]]   = rec[_header["vid"][-1]] = files.id
...
def gridMeta(context, main, add=None, filterinfo=None,
    ...
    settings["gridsettings"]["columns"]["col"].append({"@id":_header["vid"][0]})
    settings["gridsettings"]["columns"]["col"].append({"@id":_header["name"][0]})
    settings["gridsettings"]["columns"]["col"].append({"@id":_header["uploadVersioning"][0]})
